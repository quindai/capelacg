...
public class JOGL2Nehe26Reflection implements GLEventListener, KeyListener {
   ...
   public void init(GLAutoDrawable drawable) {
      ...
      try {
         for (int i = 0; i < textures.length; i++) {
            ...
            textures[i] = TextureIO.newTexture(im, false);
      ...
      // Set up lighting
      gl.glLightfv(GL2.GL_LIGHT0, GL2.GL_AMBIENT, lightAmbientValue, 0); // Set the ambient lighting for LIGHT0
      gl.glLightfv(GL2.GL_LIGHT0, GL2.GL_DIFFUSE, lightDiffuseValue, 0); // Set the diffuse lighting for LIGHT0
      gl.glLightfv(GL2.GL_LIGHT0, GL2.GL_POSITION, lightDiffusePosition, 0); // Set the position for LIGHT0
      ...
      glu.gluQuadricNormals(quadric, GL2.GL_SMOOTH); // Generate smooth normals
   ...
   public void display(GLAutoDrawable drawable) {
      ...
      gl.glEnable(GL2.GL_STENCIL_TEST);      // Enable stencil buffer for "marking" the floor
      gl.glStencilFunc(GL2.GL_ALWAYS, 1, 1); // Always passes, 1 Bit Plane, 1 As Mask
      gl.glStencilOp(GL2.GL_KEEP, GL2.GL_KEEP, GL2.GL_REPLACE); // Set the stencil buffer to 1 where we draw any polygon
      ...
      gl.glStencilOp(GL2.GL_KEEP, GL2.GL_KEEP, GL2.GL_KEEP); // Don't change the stencil buffer
      ...
      gl.glLightfv(GL2.GL_LIGHT0, GL2.GL_POSITION, lightDiffusePosition, 0); // Set up LIGHT0
      ...
      gl.glDisable(GL2.GL_STENCIL_TEST); // We don't need the stencil buffer any more

      gl.glLightfv(GL2.GL_LIGHT0, GL2.GL_POSITION, lightDiffusePosition, 0); // Set up LIGHT0 position
      gl.glEnable(GL2.GL_BLEND);     // Enable blending (otherwise the reflected object won't show)
      gl.glDisable(GL2.GL_LIGHTING); // Since we use blending, we disable Lighting
      gl.glColor4f(1.0f, 1.0f, 1.0f, 0.8f); // Set color to white with 80% alpha
      gl.glBlendFunc(GL2.GL_SRC_ALPHA, GL2.GL_ONE_MINUS_SRC_ALPHA); // Blending based on source alpha and 1 minus dest alpha
   ...
   private void drawBall(GL2 gl) {
      ...
      gl.glBlendFunc(GL2.GL_SRC_ALPHA, GL2.GL_ONE);  // Set blending mode to mix based on source alpha
      gl.glEnable(GL2.GL_TEXTURE_GEN_S);   // Enable sphere mapping on s-axis
      gl.glEnable(GL2.GL_TEXTURE_GEN_T);   // Enable sphere mapping on t-axis
